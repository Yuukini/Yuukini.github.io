 ...
 */
 ...
 function tetraedre(side) {
   ...
   face( [s/2,s,0,   0,0,0,      s,0,0    ],   [0,0,1]);
   face( [s,0,0,     s/2,s/2,s,  s/2,s,0  ],   [1,1,0]);
   face( [0,0,0,     s/2,s/2,s,  s,0,0    ],   [1,1,0]);
   face( [s/2,s,0,   s/2,s/2,s,  0,0,0    ],   [1,1,0]);
...
/**
 ...
 function wings() {
   ...
   face( [0,0,0,     0,0,10,     3,4,2.5  ],   [0,0,1]);
   face( [3,4,2.5,   0,0,10,     3,5,8    ],   [0,0,1]);
   face( [3,5,8,     0,0,10,     3,4,10   ],   [0,0,1]);
   face( [3,5,8,     3,4,10,     3,8,10   ],   [0,0,1]);
   face( [3,5,8,     3,8,10,     3,8,8    ],   [0,0,1]);
   face( [3,8,8,     3,8,10,     7,20,10  ],   [0,0,1]);
   face( [3,8,8,     7,20,10,    7,20,8   ],   [0,0,1]);
   face( [7,20,8,    7,20,10,    12,18,10 ],   [0,0,1]);
   face( [7,20,8,    12,18,10,   12,18,8  ],   [0,0,1]);
   face( [12,18,8,   12,18,10,   15,8,8   ],   [0,0,1]);//10
   face( [15,8,8,    12,18,10,   15,8,10  ],   [0,0,1]);
   face( [15,8,8,    15,8,10,    15,5,8   ],   [0,0,1]);
   face( [15,5,10,   15,5,8,     15,8,10  ],   [0,0,1]);
   face( [15,5,10,   20,5,10,    15,5,8   ],   [0,0,1]);
   face( [15,5,8,    20,5,10,    15,4,2.5 ],   [0,0,1]);//15
   face( [3,5,8,     15,5,8,     3,4,2.5  ],   [0,0,1]);
   face( [3,4,2.5,   15,5,8,     15,4,2.5 ],   [0,0,1]);
   face( [15,5,8,    3,5,8,      3,8,8    ],   [0,0,1]);
   face( [15,5,8,    3,8,8,      15,8,8   ],   [0,0,1]);
   face( [15,8,8,    3,8,8,      12,18,8  ],   [0,0,1]);//20
   face( [12,18,8,   3,8,8,      7,20,8   ],   [0,0,1]);

   face( [3,4,2.5,   15,4,2.5,   15,2,2.5 ],   [0,0,1]);
   face( [0,0,0,     3,4,2.5,    15,2,2.5 ],   [0,0,1]);
   face( [0,0,0,     15,2,2.5,   20,0,0   ],   [0,0,1]);

   face( [0,0,0,     20,0,10,    0,0,10   ],   [0,0,1]);
   face( [0,0,0,     20,0,0,     20,0,10  ],   [0,0,1]);//25
   face( [0,0,10,    20,0,10,    3,4,10   ],   [0,0,1]);
   face( [3,4,10,    20,0,10,    18,2,10  ],   [0,0,1]);
   face( [3,8,10,    3,4,10,     18,2,10  ],   [0,0,1]);
   face( [18,2,10,   20,2,10,    20,5,10  ],   [0,0,1]);
   face( [18,2,10,   20,5,10,    15,5,10  ],   [0,0,1]);//30
   face( [18,2,10,   15,5,10,    3,8,10   ],   [0,0,1]);
   face( [3,8,10,    15,5,10,    15,8,10  ],   [0,0,1]);
   face( [3,8,10,    15,8,10,    12,18,10 ],   [0,0,1]);
   face( [3,8,10,    12,18,10,   7,20,10  ],   [0,0,1]);

   face( [20,0,10,   20,0,0,     15,2,2.5 ],   [0,0,1]);//35
   face( [20,0,10,   15,2,2.5,   18,2,10  ],   [0,0,1]);
   face( [20,0,10,   18,2,10,    15,2,2.5 ],   [0,0,1]);
   face( [15,4,2.5,  20,5,10,    20,2,10  ],   [0,0,1]);
   face( [20,2,10,   15,2,2.5,   15,4,2.5 ],   [0,0,1]);
   face( [20,0,10,   15,4,2.5,   15,2,2.5 ],   [0,0,1]);//40
...
/*face ([10,-10,6,  10,-15,1.5, 10,-14,0    ],[0,0,1]);
face ([10,-10,6,  10,-14,0,   10,0,0      ],[0,0,1]);
face ([10,-10,6,  10,0,0,     10,0,10     ],[0,0,1]);*/
face ([10,0,10,   10,0,0,     10,20,12    ],[0,0,1]);
face ([10,20,12,  10,0,0,     10,20,0     ],[0,0,1]); 
face ([10,20,12,  10,20,0,    10,38,10    ],[0,0,1]);
face ([10,20,12,  10,38,10,   10,38,14    ],[0,0,1]);

face ([10,0,10,   0,0,0,      10,0,0      ],[0,0,1]);
face ([0,0,0,     10,0,10,    0,0,10      ],[0,0,1]); 
face ([0,0,10,    10,0,10,    10,20,12    ],[0,0,1]);
face ([0,0,10,    10,20,12,   0,20,12     ],[0,0,1]);
face ([0,20,12,   10,20,12,   10,38,14    ],[0,0,1]); //DHJ
face ([0,20,12,   10,38,14,   0,38,14     ],[0,0,1]); //DJI
face ([0,38,14,   10,38,14,   0,38,10     ],[0,0,1]); //IJK
face ([0,38,10,   10,38,14,   10,38,10    ],[0,0,1]); //KJL
face ([0,38,10,   10,38,10,   0,20,0      ],[0,0,1]); //KLB
face ([0,20,0,    10,38,10,   10,20,0     ],[0,0,1]); //BLF
face ([0,20,0,    10,20,0,    10,0,0      ],[0,0,1]); //BFE
face ([0,20,0,    10,0,0,     0,0,0       ],[0,0,1]); //BEA

/*face ([0,-14,0,   0,-15,1.5,  0,-10,6     ],[0,0,1]);
face ([0,-14,0,   0,-10,6,    0,0,0       ],[0,0,1]);
face ([0,0,0,     0,-10,6,    0,0,10      ],[0,0,1]);*/
face ([0,0,10,    0,0,0,      0,20,12     ],[0,0,1]);
face ([0,0,0,     0,20,12,    0,20,0      ],[0,0,1]);
face ([0,20,0,    0,20,12,    0,38,10     ],[0,0,1]);
face ([0,38,10,   0,20,12,    0,38,14     ],[0,0,1]);
...
face ([7.5,-10,6,    7.5,-15,1.5,   7.5,-14,0      ],[0,0,1]);
face ([7.5,-10,6,    7.5,-14,0,     7.5,0,0        ],[0,0,1]);
face ([7.5,-10,6,    7.5,0,0,       7.5,0,10       ],[0,0,1]);


face ([2.5,-14,0,    2.5,-15,1.5,   2.5,-10,6      ],[0,0,1]);
face ([2.5,-14,0,    2.5,-10,6,     2.5,0,0        ],[0,0,1]);
face ([2.5,0,0,      2.5,-10,6,     2.5,0,10       ],[0,0,1]);

face([7.5,-14,0,     7.5,-15,1.5,   2.5,-15,1.5    ],[0,0,1]); //RQN
face([7.5,-14,0,     2.5,-15,1.5,   2.5,-14,0      ],[0,0,1]); //RNO
face([2.5,-15,1.5,   7.5,-15,1.5,   7.5,-10,6      ],[0,0,1]); //NQP
face([2.5,-15,1.5,   7.5,-10,6,     2.5,-10,6      ],[0,0,1]); //NPM
face([2.5,-10,6,     7.5,-10,6,     7.5,0,10       ],[0,0,1]); //MPG
face([2.5,-15,1.5,   7.5,0,10,      2.5,0,10       ],[0,0,1]); //MGC
face([2.5,0,0,       7.5,0,0,       7.5,-14,0      ],[0,0,1]); //AER
face([7.5,-14,0,     2.5,-14,0,     2.5,0,0        ],[0,0,1]); //ROA
...
face ([0,8,0,     0,4,0,      0,8,6       ],[0,1,0]);  //BAF
face ([0,4,0,     0,2,2,      0,8,6       ],[0,1,0]);  //ACF
face ([0,2,2,     0,0,2,      0,8,6       ],[0,1,0]);  //CDF
face ([0,0,2,     0,0,6,      0,8,6       ],[0,1,0]);  //DEF
face ([0,8,0,     0,8,6,      2.5,8,0     ],[0,1,0]);  //BFH
face ([2.5,8,0,   0,8,6,      2.5,8,1     ],[0,1,0]);  //HFR
face ([2.5,8,1,   0,8,6,      10,8,6      ],[0,1,0]);  //RFO
face ([2.5,8,1,   10,8,6,     7.5,8,1     ],[0,1,0]);  //ROS
face ([7.5,8,1,   10,8,6,     7.5,8,0     ],[0,1,0]);  //SOI
face ([7.5,8,0,   10,8,6,     10,8,0      ],[0,1,0]);  //IOJ
face ([10,8,0,    10,8,6,     10,4,0      ],[0,1,0]);  //JOK
face ([10,4,0,    10,8,6,     10,2,2      ],[0,1,0]);  //KON
face ([10,2,2,    10,8,6,     10,0,2      ],[0,1,0]);  //NOG
face ([10,0,2,    10,8,6,     10,0,6      ],[0,1,0]);  //GOP

face ([7.5,8,0,   10,8,0,     10,4,0      ],[0,1,0]);  //IJK
face ([7.5,8,0,   10,4,0,     7.5,4,0     ],[0,1,0]);  //IKL

face ([7.5,4,0,   10,4,0,     10,2,2      ],[0,1,0]);  //LKN
face ([7.5,4,1,   7.5,4,0,    10,2,2      ],[0,1,0]);  //TLN

face ([7.5,4,1,   7.5,8,1,    7.5,8,0,    ],[0,1,0]);  //TSI
face ([7.5,4,1,   7.5,8,0,    7.5,4,0     ],[0,1,0]);  //TIL

face ([0,8,0,     2.5,8,0,    2.5,4,0     ],[0,1,0]);  //BHM
face ([0,8,0,     2.5,4,0,    0,4,0       ],[0,1,0]);  //BMA

face ([2.5,8,0,   2.5,4,0,    2.5,4,1     ],[0,1,0]);  //HMQ
face ([2.5,4,1,   2.5,8,0,    2.5,8,1     ],[0,1,0]);  //QHR

face ([0,4,0,     2.5,4,0,    0,2,2       ],[0,1,0]);  //AMC
face ([2.5,4,0,   2.5,4,1,    0,2,2       ],[0,1,0]);  //MQC

face ([7.5,8,1,   7.5,4,1,    2.5,4,1     ],[0,1,0]);  //STQ
face ([7.5,8,1,   2.5,4,1,    2.5,8,1     ],[0,1,0]);  //SQR


face ([0,2,2,     2.5,4,1,    10,2,2      ],[0,1,0]);  //CQN
face ([2.5,4,1,   7.5,4,1,    10,2,2      ],[0,1,0]);  //QTN
face ([0,2,2,     10,0,2,     0,0,2       ],[0,1,0]);  //CGD
face ([0,2,2,     10,2,2,     10,0,2      ],[0,1,0]);  //CNG
face ([0,0,2,     10,0,2,     10,0,6      ],[0,1,0]);  //DGP
face ([0,0,2,     10,0,6,     0,0,6       ],[0,1,0]);  //DPE

face ([0,0,6,     10,0,6,     10,8,6      ],[0,1,0]);  //EPO
face ([0,8,6,     0,0,6,      10,8,6       ],[0,1,0]);  //FEO
