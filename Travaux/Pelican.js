...
var numNodes = 15; // number of model identifiers
...
window.onload = function init() {
    try {
        ...
        u_renderOptionLocSpace = gl.getUniformLocation(this.progSpace, "renderingoption");
...
function initNodes(Id) {
    ...
    switch (Id) {
        ...
        case spaceShipID:
            ...
            //La texture n'est pas scale, le vaisseau devient totalement blance, et je n'arrive pas a afficher le vaisseau avec les shaders donnés avec
            figure[spaceShipID] = createNode(m, SpaceShip, cubeID, null);
            break;

        case cubeID:
            m = translate(5, 20, 0);
            m = mult(m, scale(0.5,0.5,0.5));

            figure[cubeID] = createNode(m, SpaceCube, ReflectionID, null);
            break;

        case ReflectionID:
            m = translate(20, 20, 0);

            figure[ReflectionID] = createNode(m, ReflectionCube, null, null);
            break;
    }
}
...
function SpaceShip() {
    ...
    gl.uniform4fv(u_ambientProductLocSpace, flatten(ambientProduct));
    gl.uniform4fv(u_diffuseProductLocSpace, flatten(diffuseProduct));
    gl.uniform4fv(u_specularProductLocSpace, flatten(specularProduct));
    gl.uniform1f(u_shininessLocSpace, materialShininess);

    gl.uniform4fv(u_lightPositionLocSpace, flatten(lightPosition));

    gl.uniformMatrix4fv(u_projectionLocSpace, false, flatten(projection)); // send projection matrix to the new shader program

    gl.enableVertexAttribArray(a_coordsLocSpace);
    gl.enableVertexAttribArray(a_normalLocSpace);
    gl.enableVertexAttribArray(a_texcoordLocSpace);

/*    gl.activeTexture(gl.TEXTURE8);
    gl.bindTexture(gl.TEXTURE_2D, texJupiterID);

    // Send texture number to sampler
    gl.uniform1i(u_textureLocSpace, 8);
*/
    normalmatrix = extractnormalmatrix(modelview); // always extract normalmatrix before scaling
...
function ReflectionCube() {
    ...
    // Send texture number to sampler
    gl.uniform1i(texture, 7);
    gl.uniform1f(alpha, 0.5);

    normalmatrix = extractnormalmatrix(modelview); // always extract normalmatrix before scaling
    localmodelview = mult(modelview, scale(5, 5, 5));
    box.render();
    /*  gl.disable(gl.BLEND);
      gl.depthMask(true);*/
}
...
function createModelFromObjFile(ptr) {
    ...
    model.render = function () {
        for (i = 0; i < this.numberofelements; i++) {
            ...
            gl.vertexAttribPointer(a_coordsLocSpace, 3, gl.FLOAT, false, 0, 0);
            gl.bindBuffer(gl.ARRAY_BUFFER, this.normalBuffer[i]);
            gl.vertexAttribPointer(a_normalLocSpace, 3, gl.FLOAT, false, 0, 0);
            gl.bindBuffer(gl.ARRAY_BUFFER, this.textureBuffer[i]);
            gl.vertexAttribPointer(a_texcoordLocSpace, 2, gl.FLOAT, false, 0, 0);

            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.indexBuffer[i]);

            gl.uniformMatrix4fv(u_modelviewLocSpace, false, flatten(modelview)); //--- load flattened modelview matrix
            gl.uniformMatrix3fv(u_normalmatrixLocSpace, false, flatten(normalmatrix)); //--- load flattened normal matrix
            ...
            if (this.textureFile[i] != "") {
                ...
                gl.uniform1i(u_renderOptionLocSpace, 2);
            } else {
                gl.disableVertexAttribArray(a_texcoordLocSpace);
                // assign "0" to renderingoption in fragment shader
                gl.uniform1i(u_renderOptionLocSpace, 0);				
            }
